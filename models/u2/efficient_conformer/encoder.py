...
from models.u2.transformer.positionwise_feed_forward import PositionwiseFeedForward
from models.u2.transformer.embedding import PositionalEncoding
from models.u2.transformer.embedding import RelPositionalEncoding
from models.u2.transformer.embedding import NoPositionalEncoding
from models.u2.transformer.subsampling import Conv2dSubsampling4
from models.u2.transformer.subsampling import Conv2dSubsampling6
from models.u2.transformer.subsampling import Conv2dSubsampling8
from models.u2.transformer.subsampling import LinearNoSubsampling
from models.u2.transformer.attention import MultiHeadedAttention
from models.u2.transformer.attention import RelPositionMultiHeadedAttention
from models.u2.transformer.encoder_layer import ConformerEncoderLayer

from models.u2.efficient_conformer.subsampling import Conv2dSubsampling2
from models.u2.efficient_conformer.convolution import ConvolutionModule
from models.u2.efficient_conformer.attention import GroupedRelPositionMultiHeadedAttention
from models.u2.efficient_conformer.encoder_layer import StrideConformerEncoderLayer
...
class EfficientConformerEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logging.info(f"input_layer = {input_layer}, " f"subsampling_class = {subsampling_class}")
    ...
    def forward_chunk(
        ...
        if self.global_chunk_size > 0 and real_len:
            chunk_real_len = (
                real_len // self.embed.subsampling_rate // self.calculate_downsampling_factor(self.num_blocks + 1)
            )
    ...
    def forward_chunk_by_chunk(
        self, xs: torch.Tensor, decoding_chunk_size: int, num_decoding_left_chunks: int = -1, use_onnx=False
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if use_onnx:
            ...
            cnn_cache: torch.Tensor = torch.zeros(
                (self.num_blocks, 1, self.output_size(), self.cnn_module_kernel - 1), device=xs.device
            )
        ...
        for cur in range(0, num_frames - context + 1, stride):
            ...
            if use_onnx:
                att_mask: torch.Tensor = torch.ones(
                    (1, 1, required_cache_size + decoding_chunk_size), dtype=torch.bool, device=xs.device
                )
                if cur == 0:
