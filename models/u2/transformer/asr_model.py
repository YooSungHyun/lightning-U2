...
# try:
#     import k2
#     from utils.icefall import get_texts
#     from utils.icefall import get_lattice, Nbest, one_best_decoding
# except ImportError:
#     print(
...
from models.u2.transformer.decoder import TransformerDecoder
from models.u2.transformer.encoder import TransformerEncoder
from models.u2.transformer.label_smoothing_loss import LabelSmoothingLoss
...
from utils.mask import make_pad_mask, mask_finished_preds, mask_finished_scores, subsequent_mask
...
class ASRModel(torch.nn.Module):
    ...
    def _forward_encoder(
        ...
        if simulate_streaming and decoding_chunk_size > 0:
            encoder_out, encoder_mask = self.encoder.forward_chunk_by_chunk(
                speech, decoding_chunk_size=decoding_chunk_size, num_decoding_left_chunks=num_decoding_left_chunks
    ...
    def attention_rescoring(
        ...
        hyps_pad = pad_sequence(
            [torch.tensor(hyp[0], device=device, dtype=torch.long) for hyp in hyps], True, self.ignore_id
    ...
    #     # cal attention_score
    #     hyps_pad = pad_sequence(
    #         [torch.tensor(hyp, device=device, dtype=torch.long) for hyp in hyps], True, self.ignore_id
    #     )  # (beam_size, max_hyps_len)
    #     ori_hyps_pad = hyps_pad
    ...
    #     encoder_mask = torch.ones(encoder_out.size(0), 1, encoder_out.size(1), dtype=torch.bool, device=device)
    #     # used for right to left decoder
    #     r_hyps_pad = reverse_pad_list(ori_hyps_pad, hyps_lens, self.ignore_id)
    #     r_hyps_pad, _ = add_sos_eos(r_hyps_pad, self.sos, self.eos, self.ignore_id)
    #     reverse_weight = 0.5
    ...
    #         [sum([decoder_out[i, j, hyps[i][j]] for j in range(len(hyps[i]))]) for i in range(len(hyps))],
    #         device=device,
    #     )
    #     r_decoder_scores = []
